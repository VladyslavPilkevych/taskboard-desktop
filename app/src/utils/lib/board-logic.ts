...
export function createLocalTask(title: string, description: string, existing: Task[]): Task {
  const todoTasks = existing.filter((t) => t.status === 'todo')
  const nextOrder = todoTasks.length > 0 ? Math.max(...todoTasks.map((t) => t.order)) + 1 : 0
...
export function applyToggleStatus(tasks: Task[], id: string, targetStatus: TaskStatus): Task[] {
  const task = tasks.find((t) => t.id === id)

  if (!task) return tasks

  console.log('applyToggleStatus', task?.title, task.order)

  const fromStatus = task.status
  if (fromStatus === targetStatus) return tasks

  console.log('Moving task', task.title, task.order)
...
export function applyReorder(
  ...
  if (targetStatus === activeStatus) {
    ...
    return tasks.map((t) =>
      t.status === activeStatus ? { ...t, order: orderMap.get(t.id) ?? t.order } : t,
    )
  }

  const fromTasks = tasks.filter((t) => t.status === activeStatus).sort((a, b) => a.order - b.order)

  const toTasks = tasks.filter((t) => t.status === targetStatus).sort((a, b) => a.order - b.order)
